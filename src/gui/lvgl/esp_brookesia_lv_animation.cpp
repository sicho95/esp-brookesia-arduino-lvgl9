...
#if __has_include(<src/misc/lv_anim.h>)
#   include <src/misc/lv_anim.h>
#else
#   include <misc/lv_anim.h>
#endif
#if !ESP_BROOKESIA_CONF_LVGL_ANIMATION_ENABLE_DEBUG_LOG
...
LvAnimation::~LvAnimation()
{
    if (!lv_anim_delete(_native.var, _native.exec_cb)) {
        ESP_BROOKESIA_LOGD("Delete animation failed");
    }
}
...
void LvAnimation::setStyleAttribute(const StyleAnimation &attribute) const
{
    ...
    ESP_BROOKESIA_LOGD("Param: attribute(%p)", &attribute);
...
void LvAnimation::setVariableExecutionMethod(void *variable, VariableExecutionMethod method) const
{
    ...
    ESP_BROOKESIA_LOGD("Param: variable(%p), method(%p)", variable, method);

    _execution_method = method;
    lv_anim_set_var(&_native, variable);
    lv_anim_set_custom_exec_cb(&_native, [](lv_anim_t *anim, int32_t value) {
        ESP_BROOKESIA_CHECK_NULL_EXIT(anim, "Invalid animation");

        auto user_data = static_cast<UserData *>(anim->user_data);
        ESP_BROOKESIA_CHECK_NULL_EXIT(user_data, "Invalid user data");

        auto animation = user_data->animation;
        ESP_BROOKESIA_CHECK_NULL_EXIT(animation, "Animation is not set");
...
void LvAnimation::setCompletedMethod(CompletedMethod method) const
{
    ...
    ESP_BROOKESIA_LOGD("Param: method(%p)", method);
...
void LvAnimation::setUserData(void *user_data) const
{
    ...
    ESP_BROOKESIA_LOGD("Param: user_data(%p)", user_data);
...
bool LvAnimation::start(void) const
{
    ...
    ESP_BROOKESIA_CHECK_FALSE_RETURN(!isRunning(), false, "Already started");
...
bool LvAnimation::stop(void) const
{
    ...
    ESP_BROOKESIA_CHECK_FALSE_RETURN(!isRunning(), false, "Already stopped");
