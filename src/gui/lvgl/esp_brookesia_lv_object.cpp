...
#if !ESP_BROOKESIA_CONF_LVGL_OBJECT_ENABLE_DEBUG_LOG
    ...
    _native_handle(p)
{
    ...
    ESP_BROOKESIA_CHECK_FALSE_EXIT((p != nullptr) && checkLvObjIsValid(p), "Invalid object pointer(@0x%p)", p);

    lv_obj_add_event_cb(p, [](lv_event_t *e) {
        ESP_UTILS_LOG_TRACE_ENTER();

        auto *obj = static_cast<LvObject *>(lv_event_get_user_data(e));
        ESP_BROOKESIA_CHECK_NULL_EXIT(obj, "Invalid user data");
