...
#if !ESP_BROOKESIA_CONF_CORE_MANAGER_ENABLE_DEBUG_LOG
...
bool ESP_Brookesia_CoreManager::saveAppSnapshot(ESP_Brookesia_CoreApp *app)
{
    ...
    ESP_BROOKESIA_CHECK_FALSE_RETURN(app->_active_screen != nullptr, false, "Invalid active screen");
    ...
    if ((snapshot != nullptr) &&
        ...
        ESP_BROOKESIA_CHECK_NULL_GOTO(snapshot_buffer, err, "Create snapshot buffer failed");
    }
    ...
    ESP_BROOKESIA_CHECK_FALSE_GOTO(ret == LV_RESULT_OK, err, "Take snapshot fail");
