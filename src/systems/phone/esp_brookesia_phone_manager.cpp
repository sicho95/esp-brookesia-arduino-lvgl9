...
void ESP_Brookesia_PhoneManager::onRecentsScreenGesturePressingEventCallback(lv_event_t *event)
{
    ...
    if ((drag_app_id < 0) && (manager->_recents_screen_active_app != nullptr)) {
        drag_app_id = manager->_recents_screen_active_app->getId();
    }
    data = &manager->data;
    // Check if there is a snapshot to drag
    if ((drag_app_id < 0) || !recents_screen->checkSnapshotExist(drag_app_id)) {
        return;
    }
...
void ESP_Brookesia_PhoneManager::onRecentsScreenGestureReleaseEventCallback(lv_event_t *event)
{
    ...
    if (target_app_id < 0) {
        if (manager->_recents_screen_active_app != nullptr) {
            target_app_id = manager->_recents_screen_active_app->getId();
        }
    }
    if (target_app_id < 0) {
...
process:
    ...
    if (app_event_data.type != ESP_BROOKESIA_CORE_APP_EVENT_TYPE_MAX) {
        // Keep the selection on the same carousel slot when closing, so the next app
        // naturally slides into view instead of jumping backward.
        recents_screen_active_snapshot_index = manager->getRunningAppIndexById(target_app_id);
        if (recents_screen_active_snapshot_index >= 0) {
            recents_screen_active_snapshot_index = min(recents_screen_active_snapshot_index,
                                                       manager->getRunningAppCount() - 2);
        }
